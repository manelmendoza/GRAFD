*! docver: 2.0
*!  title: Analisis preliminar por ITT ciexact
*!   html: yes
*!    log: yes
*!comment: 
version 9.1
...
<h2> Análisis Final estudio GRAFD </h2>

<p> Se ha descargado  la base de datos del estudio GRAFD de REDCAP y se ha leído para su análisis. En la base de datos había
un total de <b>`nreg'</b> casos registrados de los cuales <b>`nitt'</b> fueron considerados casos para analizar por intención a tratar y 
`nexc' fueron excluidos del estudio. De los casos excluidos <b>`nperd'</b> fueron pérdidas de seguimiento y <b>`nret'</b> retiraron el consentimiento.<b> `ncontrol'</b> casos
fueron asignados al grupo de control y <b>`nexp'</b> al grupo experimentals según la intención de tratar   </p>


<h2> Características basales  según grupo de intervención </h2>

<p> En este apartado se describen las características basales de los casos por grupo de aleatorización. Para las variables cualitativas se muestra frecuencias
y porcentajes y para las variables cuantitativas se muestran medias(desviaciones típicas) medianas ( intervalo intercuartílico) minimos/maximos e intervalo de confianza.
Aunque al tratarse de un ensayo clínico no tiene sentido calcular un p valor ya que las pacientes han sido asignadas aleatoriamente a cada grupo de la misma población y 
las diferencias observadas son debidas al azar(<font size="1"> Knol M, Groenwold R, Grobbee D. P-values in baseline tables of randomised controlled trials are 
inappropriate but still common in high impact journals. European Journal of Preventive Cardiology. 2012:19(2):231-232. doi:10.1177/1741826711421688</font>)
Se añade el p valor de un test chi cuadrado para las variables cualitativas y un t-test y U de Mann-Whitney para las variables cuantitativas. </p>
...
replace ces_ind_fallida=0 if cesarea!=. & ces_ind_fallida==.
...
if `rd' !=.{
 ...
 local lirpp:disp  %5.2f lirpp[1]
  ...
  post  relrisk  ("`nomvar'") (`pa') (`pb') (`rd') (`lird') (`lsrd') (`rpp') (`lirpp') (`lsrpp') 
   ...
   save "$dta\result_itt.dta" ,replace
